...
| Faithfulness (3-judge min)   | 94.3% (33/35) [81.4%–98.4%]   | 94.3% (33/35) [81.4%–98.4%]   | 100.0% (32/32) [89.3%–100.0%] | 96.7% (29/30) [83.3%–99.4%]   | **96.2% (127/132) [91.4%–98.4%]**     |
...
> **Footnote.** All numbers from the held-out test split (n=140; see [report §4.8](reports/report_final.md#48-held-out-test-set-results-n140)). For dev-set results used during hyperparameter tuning see [report §4.3–§4.7](reports/report_final.md#43-retrieval-hit-rate). Recall@5 denominators are pooled gold-doc Bernoulli (each Tier 1/2 case contributes 1–3 gold-doc trials). Faithfulness (3-judge min) counts a case FAITHFUL only if all three judges agree: the two Yandex judges (`yandexgpt/latest` and `yandexgpt-lite/latest`) plus the cross-vendor `openai/gpt-oss-120b:free` (OpenRouter), configured 2026-05-23. Disagreements: `cardio_40` (secondary) — the historical Stage-31 case — plus `cardio_17`, `endo_21`, `endo_38`, `infect_34` (tertiary). The 2-Yandex-judge rate of 99.2% (131/132) is retained as the upper bound under same-family bias; the 3-judge rate above is the cross-vendor lower bound. Refusal Rate uses the Stage 39 re-tuned numeric out-of-scope gate `L2_REJECT_MIN = 1.020` ([report §4.5](reports/report_final.md#45-out-of-scope-refusal-gate)); both targets (≥80% T3 recall, ≤5% T1/T2 FP) are unmet at this threshold because the in-corpus / out-of-corpus min-L2 distributions overlap heavily at the 4-specialty scale — a two-stage gate is the proposed fix ([§6 Limitation 8](reports/report_final.md#6-limitations)).
...
- **Same-vendor judge bias on faithfulness (L6) — closed 2026-05-23.** Faithfulness is now evaluated by three judges: `yandexgpt/latest`, `yandexgpt-lite/latest`, and the cross-vendor `openai/gpt-oss-120b:free` (OpenRouter). The 2-Yandex-judge rate of 99.2% (131/132) is retained as the upper bound under same-family bias; the 3-judge rate of 96.2% (127/132) [91.4%–98.4%] is the cross-vendor lower bound. The −3.0 pp drop is the empirical signal Zheng et al. 2023 predict. See [report §4.4](reports/report_final.md#44-faithfulness-generation-quality), [§5.3](reports/report_final.md#53-epistemic-bounds-of-same-family-evaluation), and [§6 Limitation 6](reports/report_final.md#6-limitations).
