...
def _remove_leading_keyword_lines(body_lines: List[str], scan_lines: int = 30) -> List[str]:
    """Drop leading KEYWORDS lines from a chunk body.

    Fixes an earlier bug where the script appended a new KEYWORDS line to chunks
    that already contained one, producing duplicates.
    """
    i = 0
    removed_any = False
...
def _strip_existing_keywords_header(lines: List[str]) -> List[str]:
    """Enforce: line1=title, line2=KEYWORDS:, line3=blank, then body without further KEYWORDS lines."""
    if not lines:
        return lines
