...
3. **Four specialists registered; two FAISS indices pending.** Four specialists are registered in `agents/registry.py` (cardiologist, endocrinologist, gastroenterologist, infectionist). Only cardiology and endocrinology have full pipelines; the gastroenterologist and infectionist corpora are committed under `data/processed/gastroenterologist/` and `data/processed/infection/` but their FAISS indices are pending — run `python multi-agent_system/build_index.py --specialty {gastroenterologist|infectionist}` to build them. Once built, the full evaluation pipeline (`evaluate_retrieval.py`, `evaluate_generation.py`, `evaluate_chunk_relevance.py`) runs against the four-specialist registry without code changes; numerical results in §4 of this report cover only the original two-specialist scope.
...
The hyperparameter grid search (K × L2 threshold, 30 combinations) was performed on the 30-case development split (§3.4) and selected K=5, L2 ≤ 1.2. The chunk size choice (400 words) and keyword-stripping were validated only under joint application — see §3.5 for the confounding caveat. Four specialist agents are now registered in `agents/registry.py` (cardiologist + endocrinologist fully evaluated; gastroenterologist + infectionist pending FAISS index builds — see §6 Limitation 3). Adding any further specialty is a single `registry.py` entry plus a corpus and FAISS index build (Fix 5). No agent code is duplicated under the new SpecialistAgent abstraction.
