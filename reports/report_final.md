...
| ambig_8 | Carcinoid heart disease (right-sided valve lesions, ↑5-HIAA) | **gastroenterologist** | cardiologist | endocrinologist | cardio, endo |
...
**LLM stays inside `valid_domains` on 13/14 cases.** The single excursion is `ambig_8` (carcinoid heart disease) where the LLM routed to **gastroenterologist** even though the case is annotated cardio×endo. Carcinoid tumours are gastrointestinal in origin (mid-gut neuroendocrine) and the routing has clinical merit — carcinoid heart disease is treated by removing the GI primary, not by valve surgery alone — but `ambig_8`'s `valid_domains` list was authored pre-Stage-31 when gastroenterology was not yet a registered specialty; the case's `valid_domains` should arguably be updated to include `gastroenterologist`. This is a dataset-curation follow-up flagged in §6 L9, not a routing bug.
...
- The single `symptom_only_ambiguous` miss is one of the 8 new Stage-38 cases. The LLM's routing fell outside `valid_domains` on one case (out of 16), a much-improved 93.8% rate compared to the prior 32-case set's 100% but on a harder mix of ambiguities (the new cases require disambiguating gastro/infect from endo/cardio purely from symptoms like "fever + jaundice" or "chronic diarrhoea + weight loss" with no disease label).
...
| Gastroenterology | 58.8% (40/68) [47.0%–69.8%] | 0.764 [0.605–0.910] | 78.6% (22/28) [60.5%–89.8%] | 42.9% (3/7) [15.8%–75.0%] |
| Infectiology | 56.3% (36/64) [44.1%–67.8%] | 0.700 [0.541–0.860] | 78.6% (22/28) [60.5%–89.8%] | 42.9% (3/7) [15.8%–75.0%] |
| **Overall** | **56.8% (162/285) [51.0%–62.5%]** | **0.704 [0.631–0.777]** | **84.6% (95/112) [76.8%–90.2%]** | **20.7% (6/29) [9.8%–38.4%]** |

*(Recall@K Wilson 95% CIs are on the pooled gold-doc Bernoulli (each Tier 1/2 case contributes 1–3 gold-doc trials; totals: cardio 76, endo 77, gastro 68, infect 64 — 285 trials across 112 annotated cases). MRR@K 95% CIs are percentile-method bootstrap intervals over the per-case reciprocal-rank vector (B=10000 resamples, RNG seed=12345; helper `_bootstrap_mean_ci` in `tests/evaluate_retrieval.py`, added Stage 27). The Refusal Rate column uses the Stage-7 numeric gate `L2_REJECT_MIN = 1.020` (re-tuned in Stage 39 — see §4.5 below) rather than the legacy zero-chunk metric.)*

> **Note on Recall@K denominators:** 112 of the 120 Tier 1/2 cases were annotated by the auto-annotator (`tests/annotate_gold_sources.py --auto`), which scans the top-20 retrieved chunks and picks up to 3 documents per case with ≥1 expected-keyword hit. The 8 cases with empty `gold_sources` are the corpus-gap cases discussed in §4.3.1: `cardio_35`, `endo_46` (pre-existing) plus `gastro_37`, `gastro_39`, `gastro_44`, `infect_14`, `infect_21`, `infect_39` (new at Stage 38). They do not contribute to Recall@K. The 29 Tier 3 cases have `gold_sources: []` by design and contribute only to the Refusal Rate column. See §6 Limitation 7 for the auto-annotator circularity caveat.

> **Why Recall@K (56.8%) is far below KeywordHitRate (84.6%):** the two metrics measure different things. KeywordHitRate counts a case as a hit if any of the 5 retrieved chunks contains any expected keyword anywhere — including adjacent, off-topic content that happens to share a common word. Recall@K is far stricter: it requires the *specific documents* containing the answer (annotated via top-20 keyword coverage, then capped at 3) to land in the *top-5* retrieval window. The 28-point gap is the part of the corpus that ranks 6–20 in retrieval order — relevant, but not surfaced at K=5.
...
By design, Tier 2 (peripheral) queries stress-test the boundaries of the knowledge base. The full Stage 38 + 39 audit (auto-annotator empty-`gold_sources` ∪ KeywordHitRate misses on T1/T2) surfaces eight cases that fail to find a keyword-matching document in the top-20 retrieval window — the auto-annotator's threshold for picking a gold doc. Three are pre-existing cardio cases; five are new gastro / infect cases at Stage 38. For all eight, retrieval surfaces *adjacent* content (chunks within `MAX_L2_DISTANCE = 1.2`) but the corpus does not contain a document that matches the specific clinical-scenario keywords.

**Cardiology (3 cases, Stage 6 baseline — preserved):**
- **`cardio_23` (Pericardial effusion with tamponade risk):** retrieval returns chunks on general echocardiography and heart failure management; corpus has no `pericardiocentesis` / drainage chunks. Fix: dedicated procedural or emergency cardiology guidelines.
- **`cardio_25` (Dressler syndrome / Post-pericardiotomy):** retrieval returns adjacent NSAID-in-stable-angina chunks; corpus has no `dressler` / `colchicine` chunks. Fix: post-operative cardiac care manuals.
...
**Gastroenterology (3 new cases, Stage 38):**
- **`gastro_37` (Acute viral gastroenteritis):** retrieval returns 20 chunks within `MAX_L2`; corpus has 50 chunks mentioning "gastroenteritis" but none match the specific keyword set (`oral rehydration`, `norovirus`, `self-limiting`, `supportive care`). Likely an auto-annotator keyword-mismatch (§6 L7), not a true coverage gap — the conditions are textbook, only the management vocabulary in the corpus is more guideline-oriented.
...
**Infectiology (3 new cases, Stage 38):**
- **`infect_14` (C. difficile colitis):** 31 chunks match `Clostridium difficile|C. difficile`, but specific keywords (`fidaxomicin`, `vancomycin oral`, `CDI`) under-represented — guidelines in the corpus are older and discuss metronidazole as first-line, not the modern preferred `fidaxomicin`. Auto-annotator keyword-mismatch.
...
| Gastroenterology | T1 | 57.6% (19/33) [40.8%–72.8%] | 45.5% (15/33) [29.8%–62.0%] | 3.0% (1/33) [0.5%–15.3%] | 100% (33/33) [89.6%–100%] | 0.708 [0.500–0.903] | 0.556 [0.319–0.778] |
...
| **Overall (T1+T2)** | — | **56.8% (162/285) [51.0%–62.5%]** | **34.4% (98/285) [29.1%–40.1%]** | **2.8% (8/285) [1.4%–5.4%]** | **100% (285/285) [98.7%–100%]** | **0.704 [0.631–0.777]** | **0.519 [0.434–0.602]** |

**Dense FAISS retrieval outperforms BM25 by 22.4 percentage points on Recall@5 overall (56.8% vs 34.4%) across all four specialties.** The gap is widest on the original Stage-6 conditions — Endocrinology T1 reaches 45.4 pp (60.6% vs 15.2%) and Cardiology T1 reaches 33.4 pp (59.0% vs 25.6%) — and narrows on the new specialties' core conditions: Gastro T1 is 12.1 pp (57.6% vs 45.5%) and Infect T1 is 18.8 pp (50.0% vs 31.2%). BM25 narrows the gap further on Tier 2 (cardio T2: 10.9 pp, gastro T2: 22.9 pp, infect T2: 18.7 pp) where exact-entity-name retrieval helps (`dressler`, `colchicine`, `pericardiocentesis` in cardio T2; `Wilson disease`, `Zollinger-Ellison` in gastro T2; `Lemierre`, `mucormycosis`, `Strongyloides` in infect T2/T3). **BM25 never overtakes FAISS on any tier of any specialty.** Random retrieval stays at 0.0–13.5% — a sanity floor. The four-specialty mean (56.8%) is within 0.6 pp of the prior 2-specialty mean (56.2%, n=153) — i.e. the four-specialty extension preserves overall retrieval headline within noise; per-cell verification in §4.10 confirms cardio/endo cells are byte-identical.
...
| Retrieval Recall@K (gold-doc Bernoulli) | 64.2% (52/81) [53.3–73.8%] | 54.1% (20/37) [38.4–69.0%] | 60.3% (47/78) [49.2–70.4%] | 52.3% (23/44) [37.9–66.2%] | 58.7% (37/63) [46.4–70.0%] | 56.5% (35/62) [44.0–68.2%] | 51.6% (33/64) [39.5–63.5%] | 60.0% (33/55) [46.8–71.9%] | *Refusal rate — see §4.3* |
...
| Gastroenterology | 80.0% (28/35) [64.1%–90.0%] | 58.8% (40/68) [47.0%–69.8%] | 5.7% (2/35) [1.6%–18.6%] |
| Infectiology | 80.0% (28/35) [64.1%–90.0%] | 56.3% (36/64) [44.1%–67.8%] | 0.0% (0/35) [0.0%–9.9%] |
| **Overall** | **85.7% (120/140) [78.9%–90.6%]** | **56.8% (162/285) [51.0%–62.5%]** | **12.9% (18/140) [8.3%–19.5%]** |
...
| Retrieval Recall@5 (pooled gold-doc) | 59.0% (23/39) [43.4–72.9%] | 54.1% (20/37) [38.4–69.0%] | 60.6% (20/33) [43.7–75.3%] | 52.3% (23/44) [37.9–66.2%] | 57.6% (19/33) [40.8–72.8%] | 60.0% (21/35) [43.6–74.4%] | 50.0% (16/32) [33.6–66.4%] | 62.5% (20/32) [45.3–77.1%] | *Refusal Rate — see §4.5* |
...
**Headline:** Routing accuracy on test = **95.7% (134/140) [91.0%–98.0%]**; FAISS Recall@5 = **56.8% (162/285) [51.0%–62.5%]**; Minimum-judge faithfulness = **99.2% (131/132) [95.8%–99.9%]**; Stage-7-style T3 refusal at the new threshold = **20.7% (6/29) [9.8%–38.4%]** with T1/T2 FP rate = 17.1%.
...
| §4.3.1 corpus-gap cases | cardio_23, cardio_25, cardio_35 ✓ | endo_46 ✓ | §4.3.1 (Stage 6) |
...
2. **Cardiology + new-specialty corpus coverage gaps — concrete case list.** §4.3.1 documents eight Tier 1/2 cases where the corpus did not contain a keyword-matching document in the top-20 retrieval window: pre-existing `cardio_23`, `cardio_25`, `cardio_35` and `endo_46`; new Stage-38 cases `gastro_37`, `gastro_39`, `gastro_44`, `infect_14`, `infect_21`, `infect_39`. The cardio + endo cases are unchanged from Stage 6 — the corpus has not been re-ingested. The new gastro + infect cases are characterised in §4.3.1; the underlying-source-material remediation list is concrete (procedural cardiology guidelines for cardio; PJI references for infect; metabolic-liver-disease references for gastro), and is the next ingestion step.
...
9. **Ambiguous-case schema drift.** `tests/data/ambiguous_cases.json` mixes two annotation conventions: cases 1–8 use only `domains`, cases 9–14 use both `valid_domains` and `domains` with identical values. The Stage 38 / 39 adversarial set (`adversarial_routing.json`) uses `valid_domains` only. A future-stage unification would pick one convention; flagged here as a follow-up.
...
- **Routing accuracy on the held-out test split: 95.7% (134/140) [Wilson 95% CI 91.0%–98.0%]** (§4.1, §4.8). Cardiology 100% (35/35), Endocrinology 97.1% (34/35), Gastroenterology 88.6% (31/35), Infectiology 97.1% (34/35). The 6 LLM misses are all defensible cross-specialty ambiguities (e.g. `gastro_37` viral gastroenteritis → infectionist; `endo_48` SGLT2-inhibitor UTI complication → infectionist), not bugs. On the 14 ambiguous cross-domain cases (§4.2) the LLM stays inside `valid_domains` on 13/14; on the 64-case adversarial set (§4.2.1) it scores 98.4% (63/64) overall, with 100% on the misspelled, non-English, and dominant-pathology-mismatch categories.
- **Retrieval Recall@K on the held-out test split: 56.8% (162/285) [Wilson 95% CI 51.0%–62.5%]** (§4.3, §4.8) across 4 specialties. Per-specialty: Cardiology 56.6% (43/76), Endocrinology 55.8% (43/77), Gastroenterology 58.8% (40/68), Infectiology 56.3% (36/64). Cardio and endo cells are byte-identical to the 2-specialty Stage-31 baseline — a verified regression check. FAISS dense retrieval outperforms BM25 by 22.4 pp overall (56.8% vs 34.4%); BM25 narrows the gap on Tier 2 cases where exact-entity-name matches help. The legacy KeywordHitRate (85.7% on test) registers any keyword co-occurrence in the top-5 window and remains a loose secondary signal.
